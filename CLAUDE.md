...
- **IP locale du PC** → si le PC change de réseau (autre Wi-Fi, partage de connexion), l'IP `192.168.1.82` actuelle (changée le 29/07/2026, était `192.168.1.77` avant) changera encore. Il faudra alors mettre à jour **à la fois** `APP_URL` dans `.env` (backend) et `_candidateHosts` dans `frontend_API-master/lib/core/network/dio_client.dart`, puis relancer les deux. Ce n'est pas automatisable sans réservation DHCP sur la box/routeur.
...
- **⚠️ Quittances jamais générées en prod (trouvé et corrigé le 13/08/2026)** → cause : `QUEUE_CONNECTION=database` mais **aucun worker `php artisan queue:work` ne tourne** sur le déploiement (Render mono-service, `Dockerfile` ne lance que `php artisan serve`). `GenerateReceiptJob` (`ShouldQueue`) restait donc indéfiniment dans la table `jobs`, jamais exécuté, et la règle A.6.4 semblait "cassée" alors que la logique métier est correcte. **Fix appliqué** : `QUEUE_CONNECTION=sync` dans `.env`/`.env.example` — les jobs (génération de quittance, et tout futur job) s'exécutent immédiatement dans la requête HTTP. Pas de worker séparé nécessaire, adapté au volume d'un projet académique. Si un vrai worker en arrière-plan devient nécessaire un jour (volume qui augmente, jobs plus lourds), revenir à `QUEUE_CONNECTION=database` **et** ajouter un process Render dédié (`php artisan queue:work`) — ne pas le faire sans ce process, sinon on retombe sur le même bug.
- **⚠️ Images/PDF invisibles en prod (trouvé le 13/08/2026)** → deux causes cumulées, toutes deux côté backend (le frontend Flutter affiche telle quelle l'URL absolue renvoyée par l'API, aucun bug constaté côté Flutter) :
  1. `php artisan storage:link` n'était lancé nulle part dans le déploiement Docker/Render → `public/storage` n'existe pas → toute URL `Storage::disk('public')->url(...)` (photos de biens, photos de maintenance, PDF contrats/quittances) renvoie un 404. **Fix appliqué** : `Dockerfile` relance `php artisan storage:link --force` à chaque démarrage du conteneur (avant `php artisan serve`), nécessaire car Render sans volume payant repart de l'image à chaque redéploiement/redémarrage.
  2. `APP_URL` doit être l'URL **publique réelle** du service Render (pas `http://192.168.1.82:8000` ni `http://localhost`, valeurs de dev) — `config/filesystems.php` construit l'URL du disque `public` directement à partir de `APP_URL`. **À faire manuellement dans le dashboard Render** (variable d'environnement, pas dans `.env` qui n'est pas commité) lors du redéploiement (chantier 8 de l'audit du 13/08/2026).
  3. **Point d'attention non résolu** : sans volume persistant payant sur Render, le disque `local` est éphémère — toute image/PDF uploadé(e) est perdue au prochain redémarrage/redéploiement du conteneur (le fix `storage:link` résout le 404 "lien manquant", pas la perte de fichiers). À traiter plus tard si besoin via le disque `s3` (déjà configuré dans `config/filesystems.php`, jamais activé) ou un volume Render payant.
